...
function sendMail($email,$v_code)
{
    ...
    //Recipients
    $mail->setFrom('carhub3567@gmail.com', 'CAR_HUB');
    $mail->addAddress($email);     //Add a recipient

    //Content
    $mail->isHTML(true);                                  //Set email format to HTML
    $mail->Subject = 'Email verification from CAR_HUB';
    $mail->Body    = "Thanks for registration!
                  Click the link below to verify email address
                  <a href='http://localhost/car_rental_project-main/verify.php?email=$email&v_code=$v_code'>verify</a>";
...
if(isset($_POST['regs']))
{
    ...
    if(empty($fname)|| empty($lname)|| empty($email)|| empty($lic)|| empty($ph)|| empty($pass) || empty($gender))
    {
        ...
        $sql="insert into users (FNAME,LNAME,EMAIL,LIC_NUM,PHONE_NUMBER,PASSWORD,GENDER,verification_code,is_verified) values('$fname','$lname','$email','$lic',$ph,'$Pass','$gender','$v_code','0')";
        if (mysqli_query($con,$sql) && sendMail($_POST['email'],$v_code)){
            echo '<script>alert("Registration Successful Check Mail to verify")</script>';
            echo '<script> window.location.href = "index.php";</script>';       
           }
        else{
...
.invalid:before {
 ...
 <div class="main">
        ...
        <form id="register" action="register.php" method="POST">    
            ...
            <span id="license-error" style="color: red; display: none;">Please enter a valid driving license number. Example format: AB12-1234567</span>
            <br><br>
...
document.getElementById("lic").addEventListener("input", function() {
    var licenseNumber = this.value.trim();
    var regex = /^[A-Za-z]{2}\d{2}-\d{7}$/;
    ...
    if (regex.test(licenseNumber)) {
        ...
        this.setCustomValidity('Please enter a valid driving license number in the format: AB12-1234567');
    }
});

document.getElementById("register").addEventListener("submit", function(event) {
    var licenseNumber = document.getElementById("lic").value.trim();
    var regex = /^[A-Za-z]{2}\d{2}-\d{7}$/;
